...
public class CDTTestRunnerClient extends TestRunnerClient {
	...
	public CDTTestRunnerClient(ITestRunSession session) {
		...
		try {
			...
			Job.createSystem("Monitor test process", (ICoreRunnable)monitor -> run(toInputStream(launch))).schedule(); //$NON-NLS-1$
		} catch (CoreException e) {
			CDTUnitTestPlugin.log(e);
		}
	}

	private static InputStream toInputStream(ILaunch launch) {
		IStreamMonitor monitor = null;
		if (launch.getProcesses().length == 1) {
			monitor = launch.getProcesses()[0].getStreamsProxy().getOutputStreamMonitor();
		}
		if (monitor == null) {
			return null;
		}
		...
		return new InputStream() {
			...
			public int read() throws IOException {
				while (!launch.isTerminated() || !content.isEmpty()) {
					if (!content.isEmpty()) {
